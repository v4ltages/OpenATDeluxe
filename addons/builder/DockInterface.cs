...
[Tool]
public partial class DockInterface : Control {
	...
	public override void _EnterTree() {
		...
		try {
			...
			if (!node.IsConnected("pressed", new Callable(this, nameof(ResetButtonPressed)))) {
				node.Connect("pressed", new Callable(this, nameof(ResetButtonPressed)));
			}

			Node nodePrepare = GetNode("Content/Scroll/TabContainer/Build Management/PrepareBuild");
			if (!nodePrepare.IsConnected("pressed", new Callable(this, nameof(PrepareBuild)))) {
				nodePrepare.Connect("pressed", new Callable(this, nameof(PrepareBuild)));
			}

			importButton = (Button)GetNode("Content/Scroll/TabContainer/Images/HSplitContainer2/ExtractImages");
			if (!importButton.IsConnected("pressed", new Callable(this, nameof(ImportImages)))) {
				importButton.Connect("pressed", new Callable(this, nameof(ImportImages)));
			}
			cancelButton = (Button)GetNode("Content/Scroll/TabContainer/Images/HSplitContainer2/CancelImport");
			if (!cancelButton.IsConnected("pressed", new Callable(this, nameof(CancelImport)))) {
				cancelButton.Connect("pressed", new Callable(this, nameof(CancelImport)));
			}
		...
		if (!gamePath.IsConnected("text_submitted", new Callable(this, nameof(EnterText)))) {
			gamePath.Connect("text_submitted", new Callable(this, nameof(EnterText)));
		}

		string savedPath = (string)SettingsManager.GetSetting(ATDPathConfig, "");
		gamePath.Text = savedPath;
		EnterText(savedPath);
		} catch (Exception) {
			// GD.Print(e.Message);
	...
	public override void _Process(double delta) {
		try {
			...
			if (isLoading && currentlyLoading != null) {
				currentlyLoading.Text = "Loading" + (((Time.GetTicksMsec() / 500) % 3 == 0) ? "." : ((Time.GetTicksMsec() / 500) % 3 == 1) ? ".." : "...");
	...
	public void EnterText(string text) {
		GD.Print("EnterText called with: '" + text + "'");
		
		// Handle null or empty path
		if (string.IsNullOrEmpty(text)) {
			GD.Print("Path is null or empty - waiting for user input");
			brokenPath.Show();
		...
		if (!ATDGameLoader.IsOriginalGamePath(text)) {
			GD.PrintErr("Path validation FAILED for: " + text);
			GD.PrintErr("Make sure the path contains 'gli/glbasis.gli' and 'room' folder");
			brokenPath.Show();
			importButton.Disabled = true;
			path = null; // Explicitly set to null if validation fails
		} else {
			GD.Print("Path validation SUCCEEDED for: " + text);
			importButton.Disabled = false;
			brokenPath.Hide();
			path = text;
			GD.Print("Static path variable set to: " + path);
	...
	private void ImportThread() {
		...
		try {
			// Validate path before loading data
			if (string.IsNullOrEmpty(path)) {
				GD.PrintErr("Error: ATD path is null or empty. Please set a valid path.");
				return;
			}
			
			// Set the path before loading data
			GFXLibrary.pathToAirlineTycoonD = path;
	...
	public void PrepareBuild() {
		...
		bool backup = ((CheckBox)GetNode("Content/Scroll/TabContainer/Build Management/Backup")).ButtonPressed;
		if (backup) {
			Copy(bPath, bPath + "BACKUP");
			System.IO.File.WriteAllText(bPath + "BACKUP/.gdignore", string.Empty);
		}

		string[] files = DirAccess.GetFiles(bPath, "*", SearchOption.AllDirectories);
	...
	public static void CopyAll(DirectoryInfo source, DirectoryInfo target) {
		DirAccess.CreateDirectory(target.FullName);

		// Copy each file into the new directory.
		foreach (FileInfo fi in source.GetFiles()) {
			fi.CopyTo(Path3D.Combine(target.FullName, fi.Name), true);
		}
